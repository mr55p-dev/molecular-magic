...
DirectoryName = '.'
PickleLoc = './GDB_Test'
PickleFile = 'Test12.pkl'
scaling_factor = 627.50960803 # Hartree to kcal/mol
Group_to_Use = 'A'
MaxBondLength = 2.0 # Angstroms
output_picklename = 'GDB_allgroundnoimagnocarban_Dec2021_G298_Test12'
HeavyAtomLimit = 2
LongBondLimit = 1.60
...
logfile = open(DirectoryName+'/'+output_picklename+'_log.txt', 'w')
...
def SelectConverged(GDBclass):
    ...
    logfile.write(''+'\n')
    logfile.write('A. Files that have failed the convergence test \n')
    
    for mol in GDBclass:
        if mol.converged == True:
            Converged += [mol]
        else:
            logfile.write(str(mol.geomfilename)+'\n')
...
def SelectFragments(GDBclass):
    ...
    logfile.write(''+'\n')
    logfile.write('B. Files that have failed the fragment test \n')
...
def RemoveLongBonds(GDBclass):
    ...
    logfile.write(''+'\n')
    logfile.write('C. Files that have failed the long bonds test \n')
...
def RemoveMolAtoms(GDBclass, AtomToRemove):
    ...
    logfile.write(''+'\n')
    logfile.write('D. Files containing Fluorine atoms \n')
...
def RemoveTetravalentNs(GDBclass):
    ...
    logfile.write(''+'\n')
    logfile.write('Remove molecules with tetravalent nitrogens \n')
...
def RemoveNonMinimas(GDBclass):
    ...
    logfile.write(''+'\n')
    logfile.write('Remove molecules with imaginary frequencies \n')
...
def RemoveCarbanions(GDBclass):
    ...
    logfile.write(''+'\n')
    logfile.write('Remove carbanions/carbocations (carbon atoms with three neighbours and all neighbouring carbon atoms have four bonds). \n')
        ...
        for OBmolatom in ob.OBMolAtomIter(molecule):
            ...
            if 'C' in atomdata and valencedata == '3':
                ...
                for neighbour_atom in ob.OBAtomAtomIter(OBmolatom):
                    ...
                    neighbour_valence = str(neighbour_atom.GetValence())
    ...
    failoutputfile = open(DirectoryName+'/'+output_picklename+'_failedcarbanion.txt', 'w')
...
def RemoveSmallMolecules(GDBclass, heavyatomlimit):
    ...
    failoutputfile = open(DirectoryName+'/'+output_picklename+'_failedsmallmolecules.txt', 'w')
    for fail_molname in FailedFilenames:
        failoutputfile.write(fail_molname+',1'+'\n')
    failoutputfile.close() 
...
def RemoveStrainedMolecules(GDBclass):
    ...
    for mol in GDBclass:
        ...
        for OBangle in ob.OBMolAngleIter(OBmol):
            ...
            central_val = OBmol.GetAtom(OBangle[0]+1).GetValence()
...
GDB_data = LoadDBFromPickle(PickleLoc+'/'+PickleFile)
...
GDB_data, Removed_GDB4, Removed_Filenames, Removed_Angles, Removed_Valences = RemoveStrainedMolecules(GDB_data)
print('\n'+'Number of samples '+str(len(GDB_data)))
print('')
...
unwanted = ['69104A1a.out', '69106A1a.out', '69107A1a.out', '69117A1a.out', '69118A1a.out', '69122A1a.out', '69123A1a.out']
...
removedstrainednamesandangles = open(DirectoryName+'/'+DirectoryName+'_'+PickleFile[:-4]+'_RemovedStrainedFilenamesAndAngles'+str(LongBondLimit)+'.txt', 'w')
...
removedstrainednamesandangles.close
...
GDBDataFile = open(DirectoryName+'/'+output_picklename+'_LongBondLimit'+str(LongBondLimit)+'.plk', 'wb')
